...
シンタックスは以下の通り。
（BNFの表記とLispの表記とがごちゃまぜになっていて読みにくいとは思うがご勘弁願いたい。）

```syntax
(resignal-bind (bind*) &body body)

bind := (condition-type-specifier (var?)&rest make-condition-arguments)

condition-type-specifier := (or condition-name compound-condition-type-specifier)
condition-name := symbol
compound-condition-type-specifier := (or (and condition-type-specifier*)
...
condition-name-form := sexp which evaluated to be condition-name
